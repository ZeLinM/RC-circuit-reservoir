...
def mackey_glass(n, tau, beta, gamma, delta_t, m):
    ...
    for i in range(tau, n - 1):
        y[i + 1] = y[i] + delta_t * (beta * y[i - tau] / (1 + y[i - tau] ** m) - gamma * y[i])
...
data = mackey_glass(n, tau, beta, gamma, delta_t, m)

data_input = data[0:-1:1]
data_target = data[1::1]

train_step = 500
initialization_step = 500
...
for k in range(0,len(data_test_pred),1):
    ...
    for l in range (0,device_num,1):
        ...
        #reservoir_state_pred[l] =  pulse_width
